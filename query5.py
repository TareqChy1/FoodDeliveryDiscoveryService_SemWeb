# completed 7 number points for the projects. 
# command for before run it in the command line: pip install rdflib requests
...
def fetch_user_preferences(rdf_url):
    ...
    # Extracting preferences from RDF data
    max_price_query = """
    ...
    return max_price, latitude, longitude, radius


def query_restaurants(endpoint_url, date, time, max_price, max_distance, reference_lat, reference_lon):
    ...
    try:
        ...
        # Sort results only by score (number of matched preferences)
...
def main():
    rdf_url = "http://193.49.165.77:3000/semweb/chy-workspace/pref-charpenay.ttl"
    max_price, latitude, longitude, radius = fetch_user_preferences(rdf_url)

    if None in [max_price, latitude, longitude, radius]:
    ...
    results = query_restaurants(endpoint_url, date, time, max_price, radius, latitude, longitude)
    ...
    if results:
        for result in results:
            ...
            score = result.get("score", "N/A")  # Added line to get the score
